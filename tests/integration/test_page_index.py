"""Test if PAGE_INDEX works, with different PRETTY_URLS settings."""
...
def test_page_index(build, output_dir, pretty_urls):  # NOQA
    ...
    assert os.path.isfile(output_path(subdir1, 'page1'))
    assert os.path.isfile(output_path(subdir1, 'page2'))
    assert os.path.isfile(output_path(subdir2, 'page3'))
    assert os.path.isfile(output_path(subdir3, 'page4'))
...
def _make_output_path(dir, name, pretty):
