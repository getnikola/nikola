...
class TestHarcodedFallbacks(unittest.TestCase):
    def test_hardcoded_fallbacks_work(self):
        ...
        if sys.platform == 'win32':
            self.assertTrue(nk.is_valid_locale(str('English')))
            self.assertTrue(nk.is_valid_locale(str('C')))
        else:
            # the 1st is desired in Travis, not a problem if fails in user host
            self.assertTrue(nk.is_valid_locale(str('en_US.utf8')))
...
class TestConfigLocale(unittest.TestCase):
    ...
    def test_implicit_fallback(self):
        ...
        sanitized_fallback = nk.valid_locale_fallback(desired_locale=locale_fallback)
        self.assertTrue(nk.is_valid_locale(sanitized_fallback))
    ...
    def test_explicit_good_default(self):
        ...
        fallback, default, locales = nk.sanitized_locales(locale_fallback,
                                                          locale_default,
                                                          LOCALES,
                                                          translations)
    ...
    def test_impicit_locale_when_default_locale_is_not_defined(self):
        ...
        fallback, default, locales = nk.sanitized_locales(locale_fallback,
                                                          ...
                                                          translations)
        if sys.platform == 'win32':
            guess_locale_for_lang = nk.guess_locale_from_lang_windows
        else:
            guess_locale_for_lang = nk.guess_locale_from_lang_linux
...
class TestTestPreconditions(unittest.TestCase):
    ...
    def test_locale_eng_availability(self):
        self.assertTrue(nk.is_valid_locale(str(loc_eng)), "META ERROR: locale for english should be valid")

    def test_locale_esp_availability(self):
        self.assertTrue(nk.is_valid_locale(str(loc_spa)), "META ERROR: locale for spanish should be valid")
